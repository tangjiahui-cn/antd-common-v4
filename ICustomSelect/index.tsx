...
import { useStateWithRef } from "../_hooks/useStateWithRef";
...
export default function ICustomSelect (props: ICustomSelectProps) {
  ...
  return (
    <Select
      ...
      filterOption={(showSearch && isLocalSearch)
        ? (filterOption
          ? getFilterOption
          : localSearchFn
        )
    ...
    >
      {all && <Select.Option key={''} value={''}>{allName}</Select.Option>}
